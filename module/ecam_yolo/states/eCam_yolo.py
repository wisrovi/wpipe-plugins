        ...
        try:
            ...
            if self.task == "detect":
                if hasattr(self.yolo_model.model, "model") and isinstance(
                    ...
                    feature_extractor_blocks = list(self.yolo_model.model.model)[:-1]

                    feature_convs = []
                    for block in feature_extractor_blocks:
                        for _, module in block.named_modules():
                            if isinstance(module, nn.Conv2d):
                                feature_convs.append(module)

                    if feature_convs:
                        target_index = int(len(feature_convs) * 0.85)
                        selected_layer = feature_convs[target_index]
                        logger.info(
                            f"Detect: Safe Neck Conv2d selected (Index {target_index}/{len(feature_convs)}): {type(selected_layer)}"
                        )
                        return [selected_layer]
            ...
            for _, module in self.yolo_model.model.named_modules():
                if "SPPF" in module.__class__.__name__:
                    logger.info(
                        f"Target layer dynamically selected (SPPF): {type(module)}"
                    )
                    return [module]
            ...
            if hasattr(self.yolo_model.model, "model") and isinstance(
                ...
                logger.info(
                    f"Sequential fallback index [-2] selected: {type(target_module)}"
                )
                return [target_module]

            raise RuntimeError(
                "No standard deep layers identified inside the architecture."
            )

        except Exception as e:
            logger.warning(
                f"Dynamic layer discovery failed, executing emergency fallback: {e}"
            )
            all_convs = [
                m
                for _, m in self.yolo_model.model.named_modules()
                if isinstance(m, nn.Conv2d)
            ]
            if all_convs:
                emergency_layer = all_convs[int(len(all_convs) * 0.5)]
                return [emergency_layer]
